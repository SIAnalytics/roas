...
def do_nia_evaluation(gt_csv_path, pred_csv_path, output_folder=None):
    ...
    # 기존 데이터셋은 15개이고, 경기장이 실내, 야외경기장으로 분리되어
    # 야외경기장만을 사용합니다.
    gts_df = gts_df[gts_df.class_id !=13]
    preds_df = preds_df[preds_df.class_id !=13]
    gts_df.loc[gts_df.class_id == 16, 'class_id'] = 13
    preds_df.loc[preds_df.class_id == 16, 'class_id'] = 13


    #gts_df.loc[gts_df.class_id == 14, 'class_id'] = 13
    #preds_df.loc[preds_df.class_id == 14, 'class_id'] = 13
    #gts_df.loc[gts_df.class_id == 16, 'class_id'] = 14
    #preds_df.loc[preds_df.class_id == 16, 'class_id'] = 14
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pred_csv_path', type=str, default='result.csv')
